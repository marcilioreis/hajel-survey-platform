...
const mapRowToSurveyEnriched = (row: Record<string, unknown>): SurveyEnriched => ({
  ...
  marginOfError: (row.margin_of_error as number | null) ?? null,
  populationSize: (row.population_size as number | null) ?? null,
  confidenceLevel: (row.confidence_level as number | null) ?? null,
  expectedProportion: (row.expected_proportion as number | null) ?? null,
